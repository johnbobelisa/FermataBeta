...
type MarkerType = 'Start' | 'Move' | 'Top';
type Hold = { x: number; y: number; type: MarkerType };
...
function App() {
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    ctx.fillText(markerType[0], canvasX, canvasY + 4);
  };
            ...
            {image ? (
              <div className="p-6">
                ...
                <div className="flex gap-4 justify-center mb-4">
                  {(['Start', 'Move', 'Top'] as MarkerType[]).map((type) => (
                    <button
                      key={type}
                      onClick={() => setMarkerType(type)}
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                        markerType === type ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'
                      }`}
                    >
                      {type}
                    </button>
                ...
                <div className="text-center">
                  <p className="text-sm text-gray-600 mb-4">
                    Click on the image to tag holds. Each hold will be labeled by type (S, M, T).
                  </p>
                  <button
